...
module.exports = ((window) => {
    ...
    const ZuckJS = function (timeline, options) {
        ...
        const translate = function (element, to, duration, ease) {
            ...
            const to3d =
                (Math.abs(to) / query("#zuck-modal").offsetWidth) *
                90 *
                direction;

            if (option("cubeEffect")) {
                const scaling =
                    to3d === 0 ? "scale(0.95)" : "scale(0.930,0.930)";
        ...
        const optionsDefault = {
            ...
            <div class="story ${get(itemData, "seen") === true ? "seen" : ""}">
              ...
              <ul class="items"><span class="storty_bagde" id="story_count">2</span></ul>
            </div>`;
                },
        ...
        const ZuckModal = () => {
            ...
            const createStoryTouchEvents = function (modalSliderElement) {
                ...
                const touchStart = function (event) {
                    ...
                    const pos = findPos(
                        query("#zuck-modal .story-viewer.viewing")
                    );
                    ...
                    if (
                        clientY < 80 ||
                        clientY > modalContainer.slideHeight - 80
                    ) {
                ...
                const touchEnd = function (event) {
                    ...
                    if (touchOffset && !touchOffset.valid) {
                        ...
                        if (delta) {
                            ...
                            if (enableMouseEvents) {
                                modalSlider.removeEventListener(
                                    "mousemove",
                                    touchMove
                                );
                                modalSlider.removeEventListener(
                        ...
                        if (nextTimer) {
                            ...
                            const storyViewerViewing = query(
                                "#zuck-modal .viewing"
                            );

                            if (storyViewerViewing && video) {
                                if (
                                    storyViewerViewing.classList.contains(
                                        "muted"
                                    )
                                ) {
      ...
      */
            ...
            try {
                ...
                zuck.data[storyId].name =
                    story.querySelector(".name").innerText;
                ...
                const navigateItemCallback = function () {
                    ...
                    each(storyViewer.querySelectorAll(".time"), (i, el) => {
                        el.innerText = timeAgo(
                            nextItem.getAttribute("data-time")
                        );
                    });
        ...
        const init = function () {
            if (timeline && timeline.querySelector(".story")) {
                each(
                    timeline.querySelectorAll(".story"),
                    (storyIndex, story) => {
                        parseStory(story);
                    }
                );
    ...
    ZuckJS.buildTimelineItem = (id, photo, name, link, lastUpdated, items) => {
        ...
        each(items, (itemIndex, itemArgs) => {
            timelineItem.items.push(
                ZuckJS.buildStoryItem.apply(ZuckJS, itemArgs)
            );
        });
